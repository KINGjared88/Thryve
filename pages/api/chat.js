...
export default async function handler(req, res) {
  ...
  if (!messages || !Array.isArray(messages) || messages.length === 0) {
    return res.status(400).json({ message: "No message(s) provided" });
  }

  try {
    // Only send the latest user message plus a system prompt for context
    const chatMessages = [
      {
        role: "system",
...
- Send us a message: https://thryvecredit.com/contact-us`
    ...
    const reply = completion.choices?.[0]?.message?.content?.trim() || "Sorry, I didn't catch that. Can you try again?";
